...
if __name__=="__main__":
    ...
    ltl_pre_set=set([i['ltl_pre'] for i in normal_data])
    n_normal_data = []
    with open('spot-5t20/spot-5t20-test.json','r') as f:
        test_data_s=json.load(f)
        test_ltl_pre_set=set([i['ltl_pre'] for i in test_data_s])
        remove_ltl_pre=ltl_pre_set.intersection(test_ltl_pre_set)
        cnt=0
        print('remove_',len(remove_ltl_pre))

        for data in normal_data:
            print('\r', cnt, end='')
            cnt+=1
            if data['ltl_pre'] not in remove_ltl_pre:
                n_normal_data.append(deepcopy(data))

    normal_data=n_normal_data
