...
def preorder_turple(f):
    ...
    if isinstance(f, LTLfAnd) or isinstance(f, LTLfUntil) or isinstance(f, LTLfOr) or isinstance(f, LTLfRelease) or isinstance(f, LTLfImplies):
        if len(f.formulas)>2:
            ret_f=preorder_turple(f.formulas[0])
            for i in range(1,len(f.formulas)):
                ret_f=(f.operator_symbol,ret_f,preorder_turple(f.formulas[i]))
            return ret_f
...
def turple_fix(f):
    if not isinstance(f,str):
        if len(f)>3 and f[0] in ['&','&&','|','||']:
            # print('tuple(_ for _ in f[2:])',f[2:])
            return (f[0],turple_fix(f[-1]),turple_fix(f[:-1]))
    return f
...
def ltl2turple(ltl: str):
    ...
    print('finish parse',time.time()-stime)
...
def check(ltl,trace,vocab):
    ...
    formula_=ltl2turple(ltl)
    formula=deepcopy(formula_)

    finish_preprocess_time=time.time()
    print('preprocess time',finish_preprocess_time-start_time)
    print(formula)
    # with open('a.txt','r') as f:
    #     formula=eval(f.readline())
    # print(formula)
    # formula=eval(input())
    if printproof:
    ...
    print('check time',finish_check_time-finish_preprocess_time)
    return value
...
if __name__ == '__main__':
    ...
    # value=check(ltl,trace,vocab.keys())
    # print('value',value)
    import code
    import json
    import sys
    ...
    input_file="declare_test.json"
    with open(input_file,"r") as f:
        datas=json.load(f)
        for data in datas:
            ltl=data['inorder']
            trace = ([['p76', 'p71', 'p21', 'p9', 'p13', 'p95', 'p53', 'p42', 'p92'],
                      ['p71', 'p9', 'p13', 'p52', 'p89', 'p42', 'p77'],
                      ['p76', 'p96', 'p82', 'p13', 'p89', 'p95', 'p93', 'p42'],
                      ['p96', 'p71', 'p9', 'p52', 'p56', 'p89', 'p42', 'p77'],
                      ['p76', 'p96', 'p82', 'p13', 'p89', 'p95', 'p93', 'p42']], 2)
            vocab=set([f'p{i}' for i in range(200)])
